...
/// プールが上限になることを避けるのが目的で、**これ自体は実測に基づく最適値ではない**。
///
/// # 未検証（重要）
///
/// **並行数16/20での実測はまだ行っていない。** 計測ハーネス
/// （`src/state/useNodeConcurrencyBench.ts`）は現在 `[1, 4, 8]` のみを測る。
/// 「どこで頭打ちになるか」「論理コア数と同じが最適か」は未確認である。
///
/// 測り直すときは ADR-0007 と同じ方法（互いに重ならないノード集合、リリースビルド）で、
/// **フロント側の同時リクエスト数とプールサイズの両方を振って**測ること。
