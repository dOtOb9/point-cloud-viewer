...
**上流の複数ページ走査そのものは正しく実装されていた。** 修正対象の if 文の直後にある
 は  を正しく使い、 は
子ページ参照を循環対策付きで辿る。**正しく動くコードの手前に、誤った前提のガードが
置かれていただけだった。**

そこで  0.9.0 を  に取り込み、そのガードを
「root ページが EVLR の範囲内に収まっているか」という正しい条件に直した（オーバーフロー検査も追加）。
ルートの  の  で差し替えている。
上流は MIT OR Apache-2.0 で本プロジェクトと同じライセンスなので、改変して取り込める。

自前で hierarchy 走査を書き直す案（方針1）は不要になった。直すべき箇所が3行だったため。
...
| **開く時間** | **0.9 ms** | **5.8 ms** |
| ルートノード読み出し | 61,201点 / 1.17 MiB / 27.8 ms | 52,296点 / 1.00 MiB / 20.1 ms |
...
CPU コストで、 ハンドラが同期版のままだと**毎秒50ノード程度が上限**になることを意味する。
M1 で観測された「読み込みが数十秒詰まる」現象の裏付けになる数値である
（[M1](./M1-point-rendering.md) の「原因の候補を特定した」を参照）。
