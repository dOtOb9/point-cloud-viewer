...
- 状態: 採択（**追記2件あり: 末尾の「copc-reader 0.9.0 は大規模 COPC を開けない」と、
  「副次的な問題と対処」節末尾の2026-09-23追記（`ADR-0007`と同根の性能問題）を
  必ず読むこと**）
- 日付: 2026-09-22
...
**追記（2026-09-23）**: この落とし穴は、後に`ADR-0007`が突き止めた性能問題
（並行数を上げても8スレッドで頭打ちになる。実は1ノードあたりの読み出しに
88msかかっていた）と**全く同じ根**だった。「そのレベル全体への空間クエリを
投げて、bboxで絞り込んでから大半を捨てる」という実装そのものが遅く、
`point_belongs_to_key`はその遅い実装が生む誤り（隣接ノードの点の混入）への
対症療法に過ぎなかった。COPCのhierarchy entryが持つ`offset`/`byte_size`へ
直接seekして、そのノード自身のLAZチャンクだけを伸長する方式
（`vendor/copc-reader`に`CopcReader::read_node`を追加。`vendor/copc-reader/PATCH.md`
参照）に変えたところ、正しさの問題（点の混入）も速度の問題（頭打ち）も
同時に解決し、`point_belongs_to_key`自体が不要になったので削除した。
詳細は`ADR-0007`の追記を参照。
...
- （2026-09-23時点では解消済み）当初は1ノード取得のための境界点判定を自前実装
  する必要があった（上記の副次的な問題）。`copc-reader` の公開APIが
  「LODレベル+bbox」という粗い粒度のクエリしか提供していないための代償だった。
  その後`vendor/copc-reader`に1エントリを直接読むAPIを追加したことで、
  この自前実装（`point_belongs_to_key`）自体が不要になり削除した
  （`ADR-0007`追記参照）。クレートを差し替える場合は同種の対処が再び要りうる
...
### 実際に採った対処（2026-09-22、解決済み）

**上流の複数ページ走査そのものは正しく実装されていた。** 修正対象の if 文の直後にある
`read_hierarchy_page_at` は `root_hier_offset` を正しく使い、`insert_hierarchy_pages` は
子ページ参照を `visited_pages` による循環対策付きで辿る。**正しく動くコードの手前に、
誤った前提のガードが置かれていただけだった。**

そこで `copc-reader` 0.9.0 を `vendor/copc-reader` に取り込み、そのガードを
「root ページが EVLR の範囲内に収まっているか」という正しい条件に直した
（オーバーフロー検査も追加）。ルートの `Cargo.toml` の `[patch.crates-io]` で差し替えている。
上流は MIT OR Apache-2.0 で本プロジェクトと同じライセンスなので、改変して取り込める。

自前で hierarchy 走査を書き直す案（上の方針1）は不要になった。直すべき箇所が3行だったため。
...
|---|---|---|
| 点数 | 10,653,336 | 364,384,576 |
| ファイルサイズ | 0.08 GB | 2.03 GB |
| hierarchy ノード | 278 | 13,163 |
| **開く時間** | **0.9 ms** | **5.8 ms** |
| ルートノード読み出し | 61,201 点 / 1.17 MiB / 27.8 ms | 52,296 点 / 1.00 MiB / 20.1 ms |

点数が34倍・ファイルサイズが25倍あるのに開く時間は6.4倍にとどまり、その差も
hierarchy のエントリ数（47倍）に由来する。**開くコストは点数にもファイルサイズにも
比例しない**という COPC + LOD 構成の主張が、数億点の実データで裏付けられた。

計測は次のコマンドで再現できる。

```bash
cargo run -p pcv-core --release --example open_bench -- \
  data/autzen-classified.copc.laz data/sofi.copc.laz
```

**副次的な観測:** ルートノード1つの読み出しに 20〜28 ms かかっている。これは LAZ 伸長の
CPU コストであり、`pcv://` のハンドラが同期版のままだと **毎秒50ノード程度が上限**になる
ことを意味する。M1 で観測された「読み込みが数十秒詰まる」現象を裏付ける数値である
（[M1](./M1-point-rendering.md) の「追記: 原因の候補を特定した」を参照）。
