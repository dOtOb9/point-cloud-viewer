...
所有者が OPPO Pad Air で Release `v0.1.0` の APK を試したところ:

- **点予算を手動で 20,000 点程度まで下げないとアプリが落ちる。** sofi・autzen とも同じ
- 20,000 点では形状の把握もできず、実用にならない
- **自動調整は機能しなかった。** 起動時の点予算がメモリ予算 1 GiB から逆算した上限
  （約2,680万点）で端末と無関係に決まっており（`ADR-0010`）、閉ループが見ているのは
  vsync に間に合ったかどうかだけなので、メモリ不足や GPU のリセットには反応できない

**原因はまだ確定していない。** 20,000〜100,000 点は Adreno 610 にとって軽い負荷で、
性能の上限というより不具合の可能性もある（コーディネーターの見立て）。所有者は GPU 側の
負荷が原因と見ており、**所有者の判断で GPU の最適化を先に進める。**

#### 進め方: 最適化の手段を「切り替えられる」形で入れる

最適化を一度に全部入れると、実機で何が効いたのか分からなくなる（上の「下げる順序」の項と同じ理由）。
そこで、**各手段を設定画面から個別に切り替えられるようにし、モバイルでは軽い側を既定値にする。**
所有者が実機で1つずつ切り替えて「落ちずに扱える点数」が変わるかを見れば、
最適化であると同時に原因の切り分けにもなる:

- どれかで上限が大きく上がる → GPU の負荷が原因だった（所有者の見立てどおり）
- どれを切り替えても変わらない → 負荷ではなく不具合。Rust 側の可視化（`panic = "unwind"` と
  logcat 出力。並行して作業中）の結果を待つ

#### 入れる手段（画質への影響が小さい順）

| 手段 | 何が軽くなるか | モバイルの既定 | デスクトップの既定 |
|---|---|---|---|
| **レンダースケール**（内部解像度 = 表示サイズ × DPR × 倍率） | 全画面パス（空・グリッド・EDL）とオフスクリーンの色・深度が画素数に比例して軽くなる | 0.5（未検証） | 1.0（現状どおり） |
| **点の形**（丸 / 四角） | 丸は `discard` を使うため、タイル方式の GPU では奥の点の描画を早めに打ち切れなくなる。四角なら `discard` 不要 | 四角 | 丸（現状どおり） |
| **EDL がオフのときの1パス描画** | オフスクリーンを経由しない。メモリ帯域の往復が1回減る | EDL オフ | EDL オン（現状どおり） |
| **UI のガラス表現**（`backdrop-filter`） | 合成時のぼかし | 切る（不透明の tint） | 現状どおり |
| **点予算の上限を端末から決める** | GPU バッファとキャッシュの量 | `navigator.deviceMemory` から逆算（未検証） | 現状どおり（1 GiB） |

空とグリッドは既に切り替えられる（背景を単色、グリッドをオフ）。所有者の確認手順にこれも含める。

**モバイルかどうかの判定**は、ADR-0009 の「vendor に依存した分岐を増やさない」に従い、
GPU 名ではなく `navigator.deviceMemory`（Chromium 系の WebView・Chrome で取れる。上限 8）と
タッチ主体の端末かどうかで決める。判定と既定値の決め方は1か所の純粋関数に閉じ込め、テストを付ける。

**現在の値は画面に出す**（上の「現在値を画面に出す」）。どの既定値が選ばれたかを所有者が
実機で確かめられないと、切り替え実験が成り立たない。

#### 今回やらないこと

- 閉ループによる自動的な段階の引き下げ（上の「下げる順序」を閉ループで回す部分）。
  まず手動の切り替えで、どれが効くかを実機で知ってから設計する
- M3-7（実機で WebGPU を確定させる）の計測そのもの

### 実施記録（2026-09-24、Sonnet）

上の「実施計画」どおり、5つの最適化手段を個別に切り替えられる形で実装した。
さらに実装の途中で所有者から実機の追加報告が2件入り、**6つめの手段
（デバイス消失からの復帰）を追加し、優先度の見立ても更新した**（下の
「実機の追加報告」参照）。**GUI・実機での確認はこのセッションではできない。**
`npm run typecheck`/`lint`/`test`/`build`とCI（`rust`/`frontend`/`invariants`/
`build`の4ジョブ、Pagesワークフロー）がすべて成功したことは確認したが、
それ以外(見た目・実際にAdreno 610で落ちなくなるか)は所有者に確認して
もらう必要がある。

#### 実機の追加報告（コーディネーター経由、2026-09-24）とそれによる優先度の更新

実装の途中で、Release `v0.1.0`のAPKを試した所有者から2件の追加報告があった。

1. **「WebGPUが失われました」とエラーバナーに出る、または画面が固まったまま
   二度と復帰しない。** GPUのデバイス消失(`device.lost`)が実際に起きている
   ことが確定した。「所有者はGPU負荷が原因と見ている」という当初の見立てが
   裏付けられ、かつ「今は復帰する経路が無い」という別の不具合も判明した
   （下の「6. デバイス消失からの復帰」で対処した）
2. **デバイス消失のメッセージが`VK_ERROR_DEVICE_LOST`(Dawnの
   `VulkanError.cpp`)だと判明した。** `VK_ERROR_OUT_OF_DEVICE_MEMORY`
   （メモリ不足）**ではなく**、GPUのハング、または1フレームの処理が
   長すぎてドライバにリセットされたことを示す。Adreno のVulkanドライバの
   不具合で特定の描画が固まっている可能性も残るとのこと

この2件目を受け、**優先度を「1フレームあたりのGPUの仕事を減らす手段」に
寄せる**よう見直した:

| 優先度 | 手段 | 理由 |
|---|---|---|
| **本命** | レンダースケール | 全画面パス(空・グリッド・EDL)とオフスクリーンの色・深度が画素数に比例して軽くなる。1フレームのGPU時間を最も直接的に削る |
| **本命** | 点の形（`discard`の除去） | タイル方式のGPU(Adreno等)で早期にピクセルを打ち切れるようになり、1フレームのフラグメント処理時間を削る |
| **本命** | EDLオフ時の1パス描画 | オフスクリーンへの描画・読み込み・合成パスの分、1フレームのメモリ帯域往復を削る |
| 引き続き入れるが今回の主因ではない見込み | 点予算の上限を端末から決める | `VK_ERROR_DEVICE_LOST`(ハング/リセット)であって`VK_ERROR_OUT_OF_DEVICE_MEMORY`ではないため、メモリ不足が主因の可能性は下がった。ただし無関係とも言い切れないため、実装自体は変えていない |
| 引き続き必要 | UIのガラス表現 | 3D canvas上のbackdrop-filterはコンポジタの負荷であり、GPUハングの原因とは別軸だが、GPU負荷全体を減らす手段として残す |
| **今回追加** | デバイス消失からの復帰 | ハングでも(メモリ不足でも)デバイスが失われること自体は起き得るため、原因を問わず「失われた後に固まったままにしない」ための対処として必要 |

**所有者が実機で1つずつ切り替えて症状が消えるかどうかが、原因の最終的な
手がかりになる。** 下の「所有者が実機で確かめる手順」に、コーディネーターの
指示どおり**1つずつ切り替えて試す順序**を記録した。

#### モバイル判定の場所と判断理由

`src/renderer/device-profile.ts`に`isMobileDevice()`/`defaultRenderSettings()`
（純粋関数、GPUもReactも知らない）としてまとめた。判定はADR-0009の
「GPU名に依存した分岐を増やさない」方針どおり、`navigator.deviceMemory`と
タッチ主体かどうか(`matchMedia("(pointer: coarse)")`)だけで行う。

判定式は「タッチ主体なら常にモバイル、そうでなくてもdeviceMemoryが4GiB以下
ならモバイル」というOR条件にした。理由: タッチ主体は操作方法からの独立した
根拠、低deviceMemoryはメモリ余裕の無さからの独立した根拠で、どちらか一方
だけでも「軽い側の既定値にすべき」と判断できると考えたため。deviceMemoryが
取得できない場合は「分からない」を「モバイル」と決めつけない(取得できない
デスクトップ利用者を誤って軽い既定値に落とさないため)。閾値の4GiBは
所有者の実機(RAM 4GB)に合わせた保守的な値で、実測して決めたものではない。

ブラウザAPIを直接読む`readDeviceProfileInput()`だけをこのファイルで唯一
DOM依存にし、判定・既定値ロジック本体はvitestで検証できるようにした
(`device-profile.test.ts`、9テスト)。

#### 1. レンダースケール

`src/renderer/render-scale.ts`の`computeCanvasBackingSize()`（純粋関数、
`render-scale.test.ts`で検証済み）で、キャンバスの描画バッファ
(`canvas.width`/`height`)だけを**「表示サイズ(CSS px) × レンダースケール」**
で計算し、CSS側(`w-full h-full`、`src/ui/ViewerPanel.tsx`)はそのまま表示
サイズを保つ。ブラウザが自動でバッファを表示サイズへ引き伸ばすため、内部
解像度だけを下げられる。深度・オフスクリーンのテクスチャ(`gpu-resources.ts`
の`resize()`)もこの解像度で作られる(呼び出し元が同じ`width`/`height`を
渡すため、変更なし)。

**ピクセル単位の値の補正**: 点のサイズ(`POINT_SIZE_PX`)とEDLの半径
(`edlRadiusPx`)は、どちらも「内部バッファのピクセル数」を基準にした値。
内部バッファを`renderScale`倍に縮小すると、CSSでの引き伸ばし倍率が
`1/renderScale`倍になるため、そのまま渡すと画面上の見た目のサイズが
`1/renderScale`倍に拡大されてしまう。`gpu-resources.ts`の`drawPoints()`/
`drawFrame()`で、渡す値をあらかじめ`renderScale`倍しておくことで補正した
(`renderScale=1.0`＝デスクトップの既定では補正が実質何もしないため、
変更前と同じ)。

##### 追記(2026-09-24、コーディネーターの指摘を受けた修正): devicePixelRatioを計算から外した

**当初の実装は「表示サイズ × devicePixelRatio × レンダースケール」だった。
これには2つの設計上の問題があり、コーディネーターの指摘で判明した。**

1. **タブレットでモバイルの既定値が実質効かない。** OPPO Pad Airの
   devicePixelRatioは2前後の見込み(未確認)。モバイルの既定`renderScale=0.5`
   を掛けると`2 × 0.5 = 1.0`倍になり、devicePixelRatioを一切考慮していな
   かった変更前(v0.1.0)と同じ内部解像度になっていた。所有者の実機の症状は
   `VK_ERROR_DEVICE_LOST`(GPUのハング)で、「1フレームのGPUの仕事を減らす」
   ことが本命の対策なのに、**その本命の手段(レンダースケール)が既定値では
   何も軽くしていなかった**
2. **デスクトップの見た目・負荷が変わってしまっていた。** 変更前は
   `useCopcViewer.ts`の`resize()`が`canvas.clientWidth`をそのまま使っており
   devicePixelRatioを考慮していなかった。表示スケール125%/150%等
   (devicePixelRatio>1)の環境では、`renderScale=1.0`(デスクトップの既定)
   でも以前より内部解像度が上がり、GPU負荷が増え、点も相対的に小さく見える
   ようになっていた。これは「デスクトップの見た目と挙動を変えない」という
   タスクシートの必須要件に反していた（自分自身の最初の報告でこの副作用に
   気づいて明記していたが、それが要件違反であるという結論までは出せて
   いなかった）

**修正**: `computeCanvasBackingSize()`からdevicePixelRatioの引数を削除し、
「表示サイズ × レンダースケール」だけにした（`render-scale.ts`/
`point-cloud-renderer.ts`の`applyCanvasSize()`）。これにより:
- デスクトップの既定(`renderScale=1.0`) → `canvas.clientWidth`そのまま、
  **変更前と完全に一致する**(devicePixelRatioが1でない環境でも副作用が無い)
- モバイルの既定(`renderScale=0.5`) → devicePixelRatioの値に関わらず、
  v0.1.0に比べて画素数が確実に1/4になる(縦横それぞれ1/2)

**高DPR画面での高精細表示(devicePixelRatioを使った鮮明な描画)は今回入れて
いない。** 「1フレームのGPU負荷を減らす」という今回の目的とは逆方向(解像度を
上げる方向)の要求であり、混ぜると「レンダースケールを下げれば必ず軽くなる」
という単純さが崩れる。欲しくなったら、`renderScale`とは独立した別の設定
(例:「高精細表示」のON/OFF)として後で足す方が筋が良いと判断した。

`render-scale.test.ts`に「renderScale=1.0ならdevicePixelRatio(に相当する
条件)に関わらずCSSサイズと完全に一致する」テストを追加した(コーディネーター
の受け入れ条件どおり)。

#### 2. 点の形（丸/四角）

`discard`を条件分岐で迂回する案ではなく、フラグメントエントリポイントを
`fs_main_round`/`fs_main_square`の2つに分け、`GpuResources`が起動時に
「丸/四角 × オフスクリーン向け/スワップチェーン向け」の4つのパイプラインを
作成しておき、`drawFrame()`が毎フレーム`options.pointShape`でどれを使うかを
選ぶだけにした(パイプラインの再作成は不要)。

**「条件分岐でdiscardを飛ばすだけでは不十分」と判断した理由**: `discard`は
実行時にどちらの分岐へ進むかに関わらず、シェーダの中に存在するというだけで
GPU（特にAdreno等のタイルベースGPU）のEarly-Z/隠面消去の最適化を無効化しうる。
`pointShape`をuniformで受け取ってif分岐でdiscardを迂回する書き方では、
コンパイラは実行時の値を静的に知らないため「discardされないことがある」
余地を消せず、四角形を選んでも最適化が有効に戻らない可能性が高い。
エントリポイントごと分ければ、四角形用のパイプラインのシェーダバイナリには
`discard`命令そのものが存在しない、という静的な保証になる。この判断は
実測で確かめたものではなく、GPUアーキテクチャの一般的な知見に基づく判断
であることを明記する。

#### 3. EDLがオフのときの1パス描画

EDLオンなら従来通り2パス(点群→オフスクリーン、背景+EDL合成→スワップ
チェーン)。EDLオフなら、点群をオフスクリーンを経由せず、背景を描いた同じ
パスの中でスワップチェーンへ直接描く1パスにした(`gpu-resources.ts`の
`drawFrame()`のif/else、`drawBackground()`/`drawPoints()`に共通処理を
切り出した)。

**深度フォーマットの一致の確認**: 以前EDL合成パイプラインが`depthStencil`
宣言を忘れて画面が真っ黒になった事故(`edl.ts`のコメント参照)を踏まえ、
新設した4つの点群パイプライン・オフスクリーンの深度テクスチャ・
スワップチェーンの深度テクスチャが**すべて`DEPTH_FORMAT`という同じ1つの
TypeScript定数を参照する**構造にした(`gpu-resources.ts`冒頭の`DEPTH_FORMAT`
宣言のコメント、および`drawFrame()`のコメントに明記)。2パス経路は
オフスクリーンの深度、1パス経路はスワップチェーンの深度を使うが、
どちらも同じ定数から作られているため、パイプラインとパスの深度
アタッチメントのフォーマットが構造的に一致する。WebGPUのパイプライン生成
やレンダーパスはブラウザ上でしか実行できないため、これを直接実行して
確かめる単体テストは書けていない(このプロジェクトにWebGPUのモックは
無い)。「同じ定数を使っている」という構造とコメントで示すに留めた。

#### 4. UI のガラス表現

`src/ui/shell/glass.ts`に`glassSurfaceClass(glassEnabled)`を追加。
`glassEnabled=false`のとき、`backdrop-blur`を含まない不透明なtint
(`OPAQUE_GLASS_SURFACE`)を返す。`LayerPanel`/`InfoPanel`/`Dock`/
`UpdateNotice`（ガラス面を使う4つのコンポーネント。`GpuErrorBanner`と
`SettingsModal`は元々意図的にガラスを使わない設計なので対象外）が
これを使うよう変更した。`glassEnabled`は点群ビューアとは無関係な純粋な
UI設定のため`CopcViewerState`には含めず、`AppShell.tsx`が直接状態を持つ。

#### 5. 点予算の上限を端末から決める

`device-profile.ts`の`defaultRenderSettings()`に、デスクトップ/モバイルの
分岐を一本化した。デスクトップは既存のADR-0010の計算式・値(1GiBから逆算、
上限26,843,545点)をそのまま使う(**変更なし**、`device-profile.test.ts`で
固定テスト済み)。モバイルは`navigator.deviceMemory`(取得できなければ
所有者の実機に合わせて4GiBを仮定)×1/64を点キャッシュのメモリ予算とし、
そこから同じ式(`pointBudgetMaxFromMemoryBudget`)で上限を逆算する。
起動直後の点予算(開始値)は、ADR-0010追記2の「楽観的に上限から始め、外したら
即座に大きく下げる」設計をそのまま踏襲し、上限と同じ値にした。

**1/64という割合は実測していない、未検証の初期値。** 所有者の実機(4GiB)で
計算すると点予算上限は約168万点になり、デスクトップの上限(約2,684万点)の
約1/16、所有者が「形状の把握もできない」と報告した20,000点よりは十分大きい
規模として選んだ。判断の詳細は`device-profile.ts`の
`MOBILE_POINT_CACHE_MEMORY_FRACTION`のコメントに書いた。

`CACHE_BUDGET_MULTIPLIER`(点予算に対するキャッシュの倍率、値は2のまま)は
`point-cloud-renderer.ts`から`point-budget.ts`へ移した。`device-profile.ts`が
`point-cloud-renderer.ts`を呼ぶことは無いが、`point-cloud-renderer.ts`は
`device-profile.ts`を呼ぶため、循環importを避けるための移動。

#### 6. デバイス消失(device.lost)からの復帰（実機の追加報告を受けて追加）

上の「実機の追加報告」1件目（デバイス消失後、画面が固まったまま二度と
復帰しない）への対処。WebGPUの推奨(MDN "Handling device loss")どおり、
`device.lost`を受けたらアダプタ・デバイスを取り直し、デバイスに紐づく
GPUリソースをすべて作り直す設計にした。

- `src/renderer/device-recovery.ts`（新設）: 「復帰を試みてよいか」を
  直近の試行頻度だけで判定する純粋関数(`decideDeviceRecovery`)と、
  試行履歴を更新する関数(`recordDeviceRecoveryAttempt`)。GPUもReactも
  知らないためvitestで検証できる(8テスト、`device-recovery.test.ts`)。
  既定は「直近60秒に3回まで」（**未検証の初期値**。所有者の実機での
  再現頻度が分からない段階の経験的な見立て）
- `src/renderer/gpu-resources.ts`: `init()`が行っていたアダプタ取得〜
  sky/grid/EDL初期化までを`setupDevice()`に切り出し、`device.lost`の
  ハンドラを`handleDeviceLost()`にした。この関数が、
  1. `reason`/`message`をそのままバナーに出す（**既存の実装で既に
     満たしていた。今回は確認のみ、変更していない**）
  2. `reason === "destroyed"`（意図的な破棄）なら復帰しない
  3. `dispose()`済みなら何もしない
  4. `decideDeviceRecovery`で頻度を判定し、諦める場合は理由もバナーに出す
  5. 試みる場合は`this.device = null`にして`isReady()`を即falseにし
     （`renderOnce()`はその場でreturnするので、復帰完了までフレームが
     安全にスキップされる）、`setupDevice()`を呼び直し、直近の描画サイズ
     (`resize()`で覚えている`lastWidth`/`lastHeight`)で深度・オフスクリーン
     テクスチャも作り直す
  6. 成功したら`onDeviceRecovered()`を呼ぶ
- `src/renderer/point-cloud-renderer.ts`: `onDeviceRecovered`コールバックで
  `this.cache.dispose()`を呼び、ノードキャッシュを空にする。**古いデバイスの
  GPUバッファ(`vertexBuffer`/`uniformBuffer`)は新しいデバイスでは使えない**
  ため。`NodeLoader`はDataSource越しの生バイト取得でデバイスに依存しない
  ため影響を受けず、キャッシュが空になったことで次のフレームから自然に
  読み込み直される

**確認できていないこと**: 実機での復帰の成否そのもの(GUI・実機が無い環境の
ため)。特に、`renderOnce()`が`isReady()`をチェックしてから復帰完了までの
間、`resize()`が(ウィンドウサイズ変更等で)呼ばれた場合に競合が無いかは、
コードレビュー上は問題ない設計にしたつもりだが(`resize()`は`device`が
無くてもサイズだけは記録するようにしてある)、実機での確認はできていない。

#### 触ったファイル

- `src/renderer/device-profile.ts`（新設）+ `.test.ts`: モバイル判定・
  各手段の既定値
- `src/renderer/render-scale.ts`（新設）+ `.test.ts`: キャンバスサイズ計算
- `src/renderer/point-budget.ts`: `CACHE_BUDGET_MULTIPLIER`をここへ移動
- `src/renderer/point-cloud-renderer.ts`: コンストラクタで端末プロファイルを
  読み、`renderScale`/`pointShape`/`edlEnabled`/点予算上限・開始値を決める。
  `resize()`/`setRenderScale()`/`setPointShape()`/`getIsMobile()`/
  `getAutoPointBudgetMax()`を新設。`RenderStats`に`renderScale`/`pointShape`/
  `isMobile`/`pointBudgetMax`を追加
- `src/renderer/gpu-resources.ts`: 点群パイプラインを4つに(丸/四角×
  オフスクリーン/スワップチェーン)、`drawFrame()`をEDLオン/オフで2パス/
  1パスに分岐、`drawBackground()`/`drawPoints()`に共通処理を切り出し。
  `init()`を`setupDevice()`に切り出し、`device.lost`から復帰する
  `handleDeviceLost()`を新設(6.参照)
- `src/renderer/device-recovery.ts`（新設）+ `.test.ts`: デバイス消失からの
  復帰を試みてよいかの頻度判定（6.参照）
- `src/state/useCopcViewer.ts`: `defaultRenderSettings()`から`edlEnabled`/
  `pointBudget`の初期値を決めるよう変更。`renderScale`/`pointShape`の状態と
  setter、`isMobile`/`deviceMemoryGiB`/`pointerCoarse`/`pointBudgetMax`の
  表示用の値を追加
- `src/ui/shell/glass.ts`: `OPAQUE_GLASS_SURFACE`/`glassSurfaceClass()`を追加
- `src/ui/shell/AppShell.tsx`: `glassEnabled`状態を新設し各パネルへ配布
- `src/ui/shell/LayerPanel.tsx`/`InfoPanel.tsx`/`Dock.tsx`/`UpdateNotice.tsx`:
  `glassEnabled` propを受け取り`glassSurfaceClass()`を使うよう変更。
  `LayerPanel`のEDLチェックボックスはSettingsModalへ移設
- `src/ui/shell/SettingsModal.tsx`: 「モバイル最適化(M3-8)」節を新設。
  端末プロファイルの判定結果と5つの手段の現在値・切り替えをまとめて表示

`useCopcViewer.ts`は並行作業(落ちたときの原因の可視化)と重なる可能性が
指摘されていたが、こまめに`git rebase origin/main`を行い、コンフリクトは
発生しなかった(node-load-error周りの変更が先にmainへ入っていたのを
rebaseで取り込んだのみ)。

#### 各手段の既定値一覧（モバイル判定時）

| 手段 | モバイルの既定 | デスクトップの既定(変更前と同じ) | 切り替え場所 |
|---|---|---|---|
| レンダースケール | 0.5 | 1.0 | 設定画面「モバイル最適化」節 |
| 点の形 | 四角 | 丸 | 同上 |
| EDL | オフ | オン | 同上（元LayerPanelから移設） |
| UIのガラス表現 | オフ(不透明tint) | オン(backdrop-blur) | 同上 |
| 点予算の上限 | deviceMemoryから逆算(4GiB時 約168万点) | 1GiB固定(約2,684万点) | UIからの直接切り替えは無し(手動で数値変更は可) |
| デバイス消失からの復帰 | 常時有効(切り替え不可) | 常時有効 | (切り替え対象外。原因を問わず必要なため) |
| 背景・グリッド(既存, M2-0c) | 変更なし(単色暗/グリッドオフが既定) | 同左 | レイヤーパネル |

#### 所有者が実機で確かめる手順

**確かめていないことを「確認した」と書かない約束のため、以下はすべて
所有者に実機で確認してほしい項目である。**

1. `git pull`して最新のAPKをビルドし、OPPO Pad Airにインストールする
   (既存のM3-1〜M3-3のワークフローで作られたAPKを使う想定)
2. 起動直後、設定画面（ドックの「設定」→「モバイル最適化(M3-8)」節）を開き、
   - 「端末プロファイル判定」が「モバイル」になっているか
   - `deviceMemory`が`4 GiB`(またはそれに近い値)、`pointer:coarse`が`true`に
     なっているか
   - レンダースケールが`0.5`、点の形が「四角」、EDLがオフ、UIのガラス表現が
     オフ、点予算の上限が約168万点になっているか(上の一覧表と一致するか)
   を確認する（見た目でなく、設定画面の表示そのものが正しく出ているかの確認）
3. **本題**: 点予算を自動調整のまま(または手動で168万点程度に)開き、
   `sofi.copc.laz`等が以前(20,000点)より大幅に多い点数で、**落ちずに**
   開けるか、また「WebGPUが失われました」のバナーが出ずに使えるかを確認する
4. **落ちる・デバイスが失われる場合、コーディネーターの指示どおり、
   以下の順序で1つずつ切り替えて症状が消えるかを見る**（`VK_ERROR_DEVICE_LOST`
   ＝ハング/リセットの可能性が高いと判明したため、「1フレームのGPUの仕事を
   減らす手段」から先に試す）:

   | 順序 | 操作 | 確認すること |
   |---|---|---|
   | 1 | 背景を単色(暗)にし、グリッドをオフにする(既存機能、既定のままのはず) | まず全画面パス(空・グリッド)が原因でないことを確認する土台を作る |
   | 2 | 設定画面でEDLがオフのままであることを確認する(モバイル既定はオフ) | 万一オンにしていた場合はここでオフに戻す。EDLは全画面パスの1つ |
   | 3 | レンダースケールを既定の0.5からさらに下げる(0.25を試す) | 症状が消える/軽くなるなら、1フレームのGPU時間(特に全画面パス)が主因の可能性が高い |
   | 4 | 点の形が既定で「四角」になっていることを確認する(モバイル既定は四角) | `discard`除去の効果を見るため、丸に戻して症状が悪化するかも比較すると分かりやすい |
   | 5 | UIのガラス表現をオン/オフで比較する(モバイル既定はオフ) | backdrop-filterの負荷がどの程度寄与するかを見る |
   | 6 | 点予算の上限を手動でさらに下げてみる(例: 50万点程度) | コーディネーターの見立てでは主因ではない可能性が高いが、念のため切り分ける |

   **各段階の結果を記録してほしい**（下の「実測の記録」に書き戻す欄がある）:
   - どの段階で症状が消える/明確に軽くなるか → その手段(またはそれ以前の
     全手段の組み合わせ)が効いている可能性が高い
   - すべて試しても変わらない → GPU負荷ではなく、Adreno のVulkanドライバの
     不具合、または別の不具合の可能性が高い。並行作業中のRust側の可視化
     (panic=unwind・logcat出力)の結果と合わせて判断する
5. **デバイス消失からの復帰**: 意図的にデバイスを消失させる操作は無いため
   直接の再現手順は無いが、実機で「WebGPUが失われました」のバナーが出た
   場合、その後**数秒待って**:
   - 画面が再び動き出す(復帰成功。バナーに「復帰しました」も出るはず)
   - バナーに「復帰を諦めました」と出て画面は固まったまま(頻発により
     上限に達した、または復帰自体が失敗した)
   のどちらかになるはずで、**以前のように何のメッセージも無く固まったまま**
   にはならないことを確認してほしい
6. デスクトップ(`npm run tauri dev`等)で、設定画面の「端末プロファイル判定」
   が「デスクトップ」になり、レンダースケール1.0・点の形「丸」・EDLオン・
   ガラスオン・点予算上限が約2,684万点のままであること(変更前と同じ)を確認
   する。**2026-09-24の修正でdevicePixelRatioを計算から外した**ため、
   表示スケール125%/150%等の環境でも内部解像度は変更前と一致するはずだが、
   念のため見た目・fpsが以前と変わっていないかも確認してほしい

### 実測の記録

> 実機での初期値・収束値・スロットリング後の値を、ここに書き戻すこと。（未計測）
> M3-8の6つの手段については、上の「所有者が実機で確かめる手順」、特に
> 「1つずつ切り替えて症状が消えるかを見る」表の各段階の結果を、
> ここに書き戻すこと（未計測）。デバイス消失からの復帰が実際に機能したか
> （バナーに「復帰しました」/「復帰を諦めました」のどちらが出たか、
> 出た場合の`reason`/`message`）も記録すること。
