...
`render-scale.test.ts`で端数の丸め・最小1px・DPRとスケールの掛け算を検証済み）
で、キャンバスの描画バッファ(`canvas.width`/`height`)だけを
「表示サイズ(CSS px) × devicePixelRatio × レンダースケール」で計算し、CSS側
(`w-full h-full`、`src/ui/ViewerPanel.tsx`)はそのまま表示サイズを保つ。
ブラウザが自動でバッファを表示サイズへ引き伸ばすため、内部解像度だけを
下げられる。深度・オフスクリーンのテクスチャ(`gpu-resources.ts`の`resize()`)
もこの解像度で作られる(呼び出し元が同じ`width`/`height`を渡すため、変更なし)。
...
**デスクトップの見た目が変わる副作用（重要、正直に書く）**: 以前は
`useCopcViewer.ts`の`resize()`呼び出しが`canvas.clientWidth`/`clientHeight`
(CSS px)をそのまま`canvas.width`/`height`に使っており、`devicePixelRatio`は
一切考慮していなかった。今回`computeCanvasBackingSize()`を導入したことで、
`renderScale=1.0`のデスクトップでも`devicePixelRatio`が初めて内部解像度に
掛かるようになった。`devicePixelRatio=1`の環境(多くの一般的なデスクトップ
モニタ)では見た目は変わらないが、`devicePixelRatio>1`の環境(Retina
ディスプレイ、Windowsの拡大縮小設定など)では、**内部解像度が上がり、
描画がより鮮明になる(かつGPU負荷も増える)。** これは「レンダースケール
という要求された仕組みを正しく実装した結果」であり、デスクトップの経路
(EDLオン・丸・ガラスあり・上限1GiB)自体は変えていないが、"見た目のピクセル
数"という意味では以前と同一ではない。**所有者の開発機のdevicePixelRatioが
1でなければ、この差が見える可能性がある。** 気になる場合は設定画面の
レンダースケールを一時的に`1/devicePixelRatio`相当にすることで、以前と
近い内部解像度に戻せる。
...
6. デスクトップ(`npm run tauri dev`等)で、設定画面の「端末プロファイル判定」
   ...
   する。**devicePixelRatioが1でない環境の場合**、上記「レンダースケール」
   節に書いた副作用（内部解像度が上がり描画がより鮮明になる／GPU負荷が
   増える可能性）が無いかもあわせて確認してほしい
