...
| 生LAS/LAZ→COPC変換（M4-3） | 実装済み: `crates/pcv-convert`（`streaming.rs`が`copc-writer`の低水準API`write_streaming_with_cancel`を呼ぶ本体。`cache.rs`/`output_path.rs`/`disk_space.rs`/`copc_detect.rs`/`crs_override.rs`/`write_metadata.rs`が判定・付随処理）と`src-tauri/src/conversion.rs`（Tauriコマンド・イベントの薄い配線）で構成する。デスクトップ・**Android**（`content://`経由）の両方で、同じプロセス内の別スレッド＋`CancelCheck`で変換しキャンセルできる（別プロセスは使わない。ADR-0006追記）。読み込み段階の進捗（点数ベース、正確な割合）とその後の段階名を`conversion-progress`イベントで通知し、`LayerPanel`が表示する。既にCOPCなら即座に開き（ヘッダー判定）、変換済みキャッシュ（指紋サイドカー）があれば再変換しない。空き容量は変換前に確認する（Windows: `GetDiskFreeSpaceExW`、Unix系/Android: `statvfs`）。CRSはWKTのVLRをそのまま運び、GeoTIFFキーのみの入力は`pcv-core`（M4-5）の検証済みパラメータからWKTを組み立てて補う。Web版はADR-0006により変換を実装せず、生LAS/LAZを選ぶとデスクトップでの変換を促す（`copc-header.ts`でヘッダー判定）。Android実機・GUIでの目視確認は未実施。[M4](./M4-import-and-conversion.md) M4-3参照 |
