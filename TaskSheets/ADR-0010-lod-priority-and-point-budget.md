...
## 追記3（2026-09-23）: リフレッシュ周期の推定側にも、同じ形の一方向ラチェットが潜んでいた

追記2で点予算の増減ロジック（AIMD）は直ったが、**その入力を作っている
`updateRefreshIntervalEstimate`自体に、今回直したのと同じ構造の、しかも
逆向きのラチェットが残っていた。** レビューで指摘され、実装前に気づけな
かった見落としである。同じ落とし穴を2回踏んだ記録として残す。

### 何が壊れていたか

`updateRefreshIntervalEstimate`は「これまで観測した最小の間隔を、それより
小さい値が来たときだけ更新する」という**単調非増加**の関数だった。これは
「一度でも詰まっていないフレームに出会えば、その間隔がほぼ正確なリフレッシュ
周期になる」という物理的な理屈に基づいていたが、**逆方向、つまり「異常に
短い間隔が一度でも来た場合」には自己修復しないことに気づいていなかった。**

異常に短い間隔は実際に起こりうる:

- ウィンドウが非表示から再表示されたとき、rAFのコールバックが詰まって
  連続発火する
- アプリ実行中に、より高いリフレッシュレートのディスプレイへウィンドウを
  移動する
- WebView2のリサイズ・オクルージョン変化に伴う不規則なrAFのタイミング

これが一度でも起きると、推定値がその小さい値（例: 2ms）に永久に固定される。
`missThresholdMultiplier`(1.5)によりミス判定の閾値が3msになり、60Hzの通常の
16.7msフレームが**すべて**ミス扱いになる。`missRatio`が常に1.0となって点予算
が0.5秒ごとに20%ずつ下がり、**上限から下限まで約11秒**（追記2で自分が計算した
秒数そのもの）で落ちる。増加条件（`missRatio ≤ hitRatioToGrow`）は推定値が
壊れている限り永久に満たせないため、二度と回復しない。所有者に見える症状は
最初のラチェットと全く同じ（「近くのチャンクが精緻にならない」）になる。

「既知の制限」として書いていた「起動直後の最初のフレームが遅い場合に固定
される」という記述は誤りだった。この関数は最小値しか更新しないので、最初が
遅くても後から速いフレームが来れば下がるだけであり、そちらは自己修復する。
危険なのは逆（短い方向）だった。この誤ったコメントは削除した。

### 新しい設計: 直近2世代（既定30秒）の最小値 + 物理的な下限のクランプ

1. **「全期間の最小値」をやめ、「直近2世代（既定`REFRESH_ESTIMATE_GENERATION_MS`
   ×2=30秒）の最小値」にした。** 1世代=15秒ごとに世代交代し、常に「現世代」と
   「1つ前の世代」の最小値の小さいほうを推定値にする。こうすると、
   - 負荷が続く区間（所有者の実機で確認された規模=数秒単位）は1世代(15秒)
     より十分短いので、最初のバグ（短いウィンドウで負荷継続区間を誤検出する）
     は再発しない
   - 異常値は高々2世代（最大30秒）で両世代から押し出され、推定値が
     **上にも戻れる**ようになる
   - 15秒・30秒という具体的な長さは、所有者の実機で確認された負荷継続時間
     （数秒）を十分に上回る値として選んだ、**実測していない未検証の初期値**
2. **物理的にありえない短さ（既定4ms=250Hz相当未満）を計測異常とみなし、
   下限にクランプしてから推定に使うようにした。** ラチェットの根本原因は
   単調非増加という構造そのものだったので窓の導入だけで直るが、加えて
   異常値がそもそも推定を汚染しにくくする二重の備えとして入れた。4msという
   値も**実測ではなく設計判断**であることをコメントに明記した

`updateRefreshIntervalEstimate`の関数シグネチャに`nowMs`（呼び出し時刻。
`requestAnimationFrame`のタイムスタンプ）を追加した。世代交代の判定に
時刻が要るための変更で、呼び出し側（`point-cloud-renderer.ts`の
`recordFrameDelta`）はすでに持っている`time`引数をそのまま渡すだけで済んだ。

### 追加・修正したテスト

`src/renderer/point-budget.test.ts`に追加した（合計19テスト）:

1. **【最重要】異常に短い間隔を1回混ぜた後、通常の間隔が続く列を与えると、
   推定値が元の水準へ戻ること。** 直す前の実装（全期間の最小値）ではこの
   テストは必ず失敗する（異常値に永久に固定される）
2. 推定が異常値で壊れても、点予算が下限に張り付いたまま戻らなくならないこと
   （`updateRefreshIntervalEstimate`と`evaluatePointBudget`を組み合わせた
   結合的なテスト）

既存の「一度小さい値を観測したら…下がらない」テストは、「同じ世代の中では」
という条件付きの主張に書き直した（世代を跨いだ場合の挙動は上記の新規テストが
担保する）。所有者の実機で確認された負荷継続時間（数秒）を明示的に使うように
テストの数値も調整した。
...
npm run typecheck   # 通ること
npm run lint        # 通ること
npm test            # 105件（このコミット時点）すべてpassすること
npm run build
```

`point-budget.test.ts`の該当テストのコメントに、何を確認しているかを
書いてあるので、疑わしければテストのアサーションを直接読んで確認できる。

**GUIでの目視確認はこの環境ではできなかった。** 今回の修正は「異常に短い
rAF間隔が一度発生した後も自動調整が壊れたままにならない」ことが主眼なので、
実機で再現するのが難しい（ウィンドウの最小化からの復帰、外部モニタの
