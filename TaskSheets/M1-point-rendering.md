...
- [ ] **`pcv://` の並行リクエストが直列化していないこと。**
      上記「追記: 原因の候補を特定した」を参照。**数億点ではノードの取得回数が桁違いに
      増えるため、直列化したままだと sofi では致命的になる。** 20万点で「多少詰まる」で
      済んでいたものが、実データでは使えないレベルになる可能性が高い。
      M2 の先頭で、並行リクエスト時のスループットを計測してから直す
