 ...
 * 上方向の既定値。
 *
 * M2-0b 段階3: `[0, 0, 1]`（Z-up）にした。LAS/COPCは投影座標系なのでZが標高であり、
 * これが点群データの実際の上方向。段階1・段階2では意図的に`[0, 1, 0]`のまま
 * 残し、空の背景を先に入れてから上方向を直すことで、この1行の変更だけで
 * 地平線の向きが90度変わる様子を単独で確認できるようにしていた（所有者の実機
 * 確認で「上から俯瞰したように見える」ことを確認済み。この報告自体が上方向が
 * Zではなく誤ってYになっていたことの裏付けになっている）。
 *
 * PLY/PCDのように座標系を持たないデータ（ADR-0008）ではこの既定が合わない場合が
 * あるので、`OrbitCamera.setUpAxis()`で変更できるようにしてある（定数ではなく
 * 設定可能な値）。
 */
export const DEFAULT_UP_AXIS: Vec3 = [0, 0, 1];
