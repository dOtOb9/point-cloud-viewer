...
export interface DrawFrameOptions {
   ...
   * devicePixelRatioは含めない。2026-09-24追記、理由はrender-scale.ts冒頭の
   * コメント参照)。点のサイズ・EDLの半径はどちらも「内部バッファのピクセル数」
   * を基準にした値なので、スケールを掛けて補正しないと、スケールを下げたときに
   * 画面上の見た目のサイズが変わってしまう(補正の理由はdrawPoints()のコメント
   * 参照)。デスクトップの既定値1.0では、この補正は実質何もしない(変更前と同じ)。
   */
  renderScale: number;
