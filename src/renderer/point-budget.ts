 ...
 * に送る）: アプリ実行中にウィンドウを別のリフレッシュレートのディスプレイへ
...
export const DEFAULT_POINT_BUDGET_TUNING: Omit<PointBudgetTuning, "limits"> = {
  ...
  missRatioToShrink: 0.1, // 未検証の初期値（直近ウィンドウの10%以上がミスなら「外した」）
  hitRatioToGrow: 0, // 未検証の初期値（ミスフレームが0のときだけ「間に合っている」を積む）
  sustainedHitsToGrow: 6, // 未検証の初期値（500ms間隔の呼び出しを想定し、3秒間の持続を要求）
