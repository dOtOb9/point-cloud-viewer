...
/**
 * M1-3: COPCビューアの状態。src/renderer と src/datasource を束ね、UI(src/ui)へは
 * このフックだけを見せる（規約3: UIはrendererを直接触らずstateを経由する）。
 * この時点ではルートノード1つだけを表示する（LODはM1-4）。
 */
export function useCopcViewer(): [RefObject<HTMLCanvasElement | null>, CopcViewerState] {
  ...
  const openFile = useCallback(async (path: string) => {
    ...
    try {
      ...
      const root = opened.nodes.find((n) => n.key === "0-0-0-0");
      if (!root) {
        throw new Error("root node (0-0-0-0) not found in hierarchy");
      }
      await renderer.loadRootNode(root.boundsMin, root.boundsMax);
      setCloudInfo(opened.info);
      setNodeCount(opened.nodes.length);
      setStatus("ready");

      const summary = `[M1] opened ${path}: points=${opened.info.pointCount} nodes=${opened.nodes.length} rootPoints=${root.pointCount}`;
      console.log(summary);
  ...
  const state: CopcViewerState = { status, error, cloudInfo, nodeCount, openFile };
