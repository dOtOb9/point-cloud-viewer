...
impl CopcFile {
    ...
    pub fn from_reader<R: Read + Seek>(reader: &mut R) -> Result<Self> {
        ...
        // PATCHED (point-cloud-viewer): 上流は root_hier_offset が hierarchy EVLR の
        // データ先頭と「一致する」ことを要求していたが、これは COPC 仕様上の誤った前提である。
        // hierarchy は EVLR 内で複数ページに分割されうるので、root ページが EVLR の先頭に
        // 来るとは限らない (例: sofi.copc.laz / 3.6億点 は root ページが EVLR 先頭 +338208)。
        // 正しい条件は「root ページが EVLR の範囲内に収まっていること」。
        // 詳細は TaskSheets/ADR-0003-copc-crate.md の追記を参照。
        let hier_end = root_evlr
            .data_offset
            .checked_add(root_evlr.data_len)
            .ok_or_else(|| Error::InvalidData("hierarchy EVLR range overflows".into()))?;
        let root_end = copc_info
            .root_hier_offset
            .checked_add(copc_info.root_hier_size)
            .ok_or_else(|| Error::InvalidData("COPC root hierarchy range overflows".into()))?;
        if copc_info.root_hier_offset < root_evlr.data_offset || root_end > hier_end {
            return Err(Error::InvalidData(format!(
                "COPC root hierarchy range {}..{} is outside the hierarchy EVLR range {}..{}",
                copc_info.root_hier_offset, root_end, root_evlr.data_offset, hier_end
            )));
        }
